...
async function loadSessionDrivers(roundNum) {
  ...
  try {
    const res = await fetch(`/drivers/2026/${roundNum}/${document.getElementById('telemetry-session').value}`);
    ...
    await fetchLiveTiming(); await fetchTelemetryForDrivers();
...
async function fetchTelemetryForDrivers() {
  ...
  try {
    const [res1, res2] = await Promise.all([
      fetch(`/telemetry/2026/${currentRoundNum}/${document.getElementById('telemetry-session').value}/${d1}`),
      fetch(`/telemetry/2026/${currentRoundNum}/${document.getElementById('telemetry-session').value}/${d2}`)
    ]);
