...
def get_race_simulation(year: int, event: str, session_type: str, drivers: str = None):
    try:
        ...
        # 3. Laps data for ALL drivers (not just selected)
        laps_dict = {}
        for drv in list(session.drivers):
...
# GET | 특정 연도 개최 이벤트
@app.get("/Grand-Prix/{year}", response_model=EventListResponse)
